...
import com.sun.xml.ws.db.xmlbeans.JWSAnnotationReader;
...
abstract public class WsDatabindingTestBase extends TestCase {
    ...
    static public <T> T createProxy(Class<T> proxySEI, Class<?> endpointClass, String db, boolean debug) throws Exception {
        Databinding.Builder sbuilder = factory.createBuilder(proxySEI, endpointClass);
        DatabindingModeFeature dbf = new DatabindingModeFeature(db); 
        sbuilder.feature(dbf);
        sbuilder.property("com.sun.xml.ws.api.model.SuppressDocLitWrapperGeneration", true);
        sbuilder.property("com.sun.xml.ws.api.model.DocWrappeeNamespapceQualified",   true);
        sbuilder.property(MetadataReader.class.getName(), new JWSAnnotationReader());


        Databinding.Builder cbuilder = factory.createBuilder(proxySEI, proxySEI);
        cbuilder.feature(dbf);
        cbuilder.property("com.sun.xml.ws.api.model.SuppressDocLitWrapperGeneration", true);
        cbuilder.property("com.sun.xml.ws.api.model.DocWrappeeNamespapceQualified",   true);
        cbuilder.property(MetadataReader.class.getName(), new JWSAnnotationReader());
        return createProxy(proxySEI, endpointClass, sbuilder, cbuilder, debug);
    }
    
    static public <T> T createProxy(Class<T> proxySEI, Class endpointClass, 
            Databinding.Builder sbuilder, Databinding.Builder cbuilder, boolean debug) throws Exception {   
        Databinding srvDb = sbuilder.build();    
        WSDLGenerator wsdlGen = sbuilder.createWSDLGenerator();     
        InVmWSDLResolver result = new InVmWSDLResolver();
        wsdlGen.inlineSchema(true);
        wsdlGen.generate(result);
...
//        srvDb.generateWSDL(wsdlGenInfo);
        ...
        cbuilder.property(WSDLPort.class.getName(), wsdlPort);
        cbuilder.serviceName(serviceName);
...
//        cliConfig.getMappingInfo().setServiceName(serviceName);
        Databinding cliDb = (Databinding)(Databinding)cbuilder.build();                
    ...
    static public void setSaajTransformation(Object proxy, boolean value) {
        WsDatabindingTestFacade h = (WsDatabindingTestFacade) Proxy.getInvocationHandler(proxy);
        h.transformToSAAJ = value;
    }
    ...
    static public class WsDatabindingTestFacade implements InvocationHandler {
        ...
        private boolean transformToSAAJ = true;
...
//if (transformToSAAJ) cliSoapReq.getAsSOAPMessage();    
            
            ByteArrayOutputStream cliBo = new ByteArrayOutputStream();
            ContentType cliCt = ((com.sun.xml.ws.api.databinding.Databinding)cli).encode(cliSoapReq, cliBo);
            if (wireLog) {
...
//          packet.soapAction = fixQuotesAroundSoapAction(con.getRequestHeader("SOAPAction"));
            ((com.sun.xml.ws.api.databinding.Databinding)srv).decode(srvBi, cliCt.getContentType(), srvSoapReq);
if (transformToSAAJ) srvSoapReq.getAsSOAPMessage();            
...
//          Packet srvSoapRes = srvSoapReq.createResponse(srvSoapResMsg);
            ...
            ContentType srvCt = ((com.sun.xml.ws.api.databinding.Databinding)srv).encode(srvSoapRes, srvBo);
            if (wireLog) {
            ...
            ((com.sun.xml.ws.api.databinding.Databinding)cli).decode(cliBi, srvCt.getContentType(), cliSoapRes);
if (transformToSAAJ) cliSoapRes.getAsSOAPMessage();            
