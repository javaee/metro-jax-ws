Sometimes, it is convenient to refer to local resources instead of referring to resources over a network.
For example, a WSDL may be accessible during client generation but not accessible when the client is run.
We can address this using XML catalogs. JAX-WS runtime uses the XML catalogs for resolving Web Service document
that is part of the description of a Web service, specifically WSDL and XML Schema documents.
